...
import { adminCreateContest } from '@/api/admin'
...
function handleProblemSelect(ids: number[]) {
  form.problemIds = ids
  // Initialize scores for new problems
...
async function handleSubmit() {
  ...
  try {
    ...
    await adminCreateContest(data as any)
...
<template>
  <div class="admin-contest-create">
    <div class="page-header">
      <h1 class="page-title">{{ t('admin.createContest') }}</h1>
    </div>
